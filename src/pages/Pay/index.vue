  ...
  <div class="pay-main">
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          <span class="fl">请您在提交订单<em class="orange time">4小时</em>之内完成支付，超时订单会自动取消。订单号：<em>{{tradeNo}}</em></span>
          <span class="fr"><em class="lead">应付金额：</em><em class="orange money">￥{{tradeInfo.totalFee}}</em></span>
        </div>
      </div>
...
import QRCode from 'qrcode'
  export default {
    ...
    beforeMount (){
      this.tradeNo = this.$route.query.orderId;
    },
    mounted(){
      this.getPayInfo();
    },
    methods : {
      async getPayInfo(){
        try{
          const result = await this.$API.reqPayInfo(this.tradeNo);
          if(result.code == 200){
      ...
      async pay(){
        try{
            const imgUrl = await QRCode.toDataURL(this.tradeInfo.codeUrl);
