<template>
  <div class="trade-container">
    ...
    <div class="content" >
      ...
      :key="userAddress.id"
      @click="changeDefault(userAddress,userAddressList)"
      >
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="detailArray in detailArrayList" :key="detailArray.skuId">
          <li>
            <img :src="detailArray.imgUrl" style="width: 100px; height: 80px" alt="" />
          </li>
          <li>
            <p>
              {{detailArray.skuName}}
...
export default {
  ...
  methods: {
    ...
    async subOrder(){
      let traderNo = this.tradeInfo.tradeNo;
      let tradeData = {
        consignee : this.userAddressList.consignee,
        consigneeTel : this.userAddressList.phoneNum,
        deliveryAddress : this.userAddressList.userAddress,
        paymentWay : "ONLINE",
        orderComment : this.messageText,
        orderDetailList : this.detailArrayList
      }
      try{
        const result = await this.$API.reqOrderInfo(tradeData,traderNo);
        console.log(result);
        console.log(1111111);
        if(result.code == 200){
          console.log(333);
  ...
  computed: {
    ...
    ...mapState({
      tradeInfo: (state) => 
        state.trade.tradeInfo
    }),
    defaultUserAddress(){
      console.log(this.userAddressList.find((item) => item.isDefault == '1'));
